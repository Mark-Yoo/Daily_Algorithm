...
function specialSort(array) {
  const copiedArr = array.slice('').sort((a, b) => a - b);
  let tempArrlow = [];
  let tempArrhigh = [];
  for (let i = 0; i < copiedArr.length; i++) {
    if (copiedArr[i] < 0) {
      tempArrlow.push(copiedArr[i]);
    } else if (copiedArr[i] >= 0) {
      tempArrhigh.push(copiedArr[i]);
    }
  }
  return [...tempArrlow.sort((a, b) => b - a), ...tempArrhigh];
}
